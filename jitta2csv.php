<?php
	...
	preg_match_all('/<div class="Text-gbmmix-1 fDxapP" color="#6c7172">฿(.+)<\/div>/U', $jitta_html, $arr_price);
	...
	$arr_price_len = count($arr_price[1]);
	
	//Out put it to csv if all the arrays are the same. 
	if( $ticker_len == $jitta_score_len && $jitta_score_len == $percentage_to_line_len && $percentage_to_line_len == $above_or_below_len && $above_or_below_len == $arr_loss_chance_len && $arr_loss_chance_len==$arr_price_len){
		$myfile = fopen("output.csv", "w") or die("Unable to open file!");
		for($i=0;$i<$ticker_len;$i++){
			fwrite($myfile, $arr_ticker[1][$i].','.$arr_jitta_score[1][$i].','.$arr_percentage_to_line[2][$i].','.$arr_above_below_line[1][$i].','.$arr_loss_chance[1][$i].','.$arr_price[1][$i]."\n");
		}
